...
fn built_in_models(auth: &AuthStorage, mode: ModelRegistryLoadMode) -> Vec<ModelEntry> {
    ...
    for legacy in legacy_generated_models() {
        ...
        if !seen.insert(dedupe_key) {
            // First entry wins — aliases like "auto" appear before their
            // canonical form "openrouter/auto" in the legacy file, and the
            // test suite expects the first entry's display name to survive.
            continue;
        }
